[![CircleCI](https://circleci.com/gh/twofortyfouram/android-monorepo.svg?style=svg)](https://circleci.com/gh/twofortyfouram/android-monorepo)
...
Although this pattern may change in the future, this seems to be a reasonable balance given how frequently the Android framework changes, thus requiring changes to these libraries.

# Building
The entire repo can be easily checked out and built locally by running `./gradlew assemble`.

# Forking and CI
Continuous Integration is only enabled for members of the project. Forks and pull requests made by non-members will not see CI builds complete.
To enable CircleCI continous integration (CI) in a fork:

1. Create a [Firebase](https://firebase.google.com) project
1. Configure the Firebase project with a [service account](https://firebase.google.com/docs/test-lab/continuous)
1. Create a CircleCI project for the fork
...
* Support for LIMIT clauses via the query parameter [SearchManager.SUGGEST_PARAMETER_LIMIT](http://developer.android.com/reference/android/app/SearchManager.html#SUGGEST_PARAMETER_LIMIT)
* Support for [BaseColumns._COUNT](http://developer.android.com/reference/android/provider/BaseColumns.html#_COUNT) queries
* Enhanced security by ensuring _ID queries are not susceptible to SQL injection
* Enhanced security by using projection maps for queries


## Usage
### Step by step
1. Define the Contract classes for the ContentProvider.  (A Contract class more or less represents a database table.)
1. Subclass [AbstractSqliteContentProvider](http://twofortyfouram.github.io/android-memento/com/twofortyfouram/memento/provider/sqlite/AbstractSqliteContentProvider.html), providing implementations for:
    1. [SqliteOpenHelper](http://developer.android.com/reference/android/database/sqlite/SQLiteOpenHelper.html): Opens and creates the database tables.  To simplify database table creation, consider using the helper classes [SqliteTableBuilder](http://twofortyfouram.github.io/android-memento/com/twofortyfouram/memento/provider/sqlite/SqliteTableBuilder.html) and [SqliteColumnBuilder](http://twofortyfouram.github.io/android-memento/com/twofortyfouram/memento/provider/sqlite/SqliteColumnBuilder.html).  Advanced users might also create indexes for improved performance via [SqliteIndexBuilder](http://twofortyfouram.github.io/android-memento/com/twofortyfouram/memento/provider/sqlite/SqliteIndexBuilder.html).
    1. [SqliteUriMatcher](http://twofortyfouram.github.io/android-memento/com/twofortyfouram/memento/provider/sqlite/SqliteUriMatcher.html): Takes Uris and converts them into [SqliteUriMatch](http://twofortyfouram.github.io/android-memento/com/twofortyfouram/memento/provider/sqlite/SqliteUriMatch.html) objects that the ContentProvider uses whenever an operation (query, insert, update, delete, etc.) occurs.
1. Create an AndroidManifest entry for the ContentProvider.

### Example
An [example implementation](https://github.com/twofortyfouram/android-memento/tree/master/mementoLib/src/androidTest/java/com/twofortyfouram/memento/test) exists as part of the test suite.

### Further reading
The official Android [ContentProvider Developer Guide](http://developer.android.com/guide/topics/providers/content-providers.html).
...
And the dependencies section would look something like this:
    ...
    dependencies {
        compile group:'com.twofortyfouram', name:'android-memento', version:’[1.0.0,2.0[‘
    }
...
The plug-in architecture is implemented in three different layers (API, SDK, and Example), with each subsequent layer becoming more abstract and easier to work with.  Different sections of this document describe each of those layers in detail.
## pluginApiLib
This repo contains the definition for that Intent-based API.  The Intent-based API is the lowest level layer of the plug-in API for Locale.
...
### Compatibility
The SDKs are compatible and optimized for Android API Level 9 and above.  Note, however, consuming the SDKs requires at least Android Gradle Plugin 1.3.0. (This is due to a manifest placeholder in the AAR's manifest.)  For backwards compatibility, there are support-v4 versions of the Fragment and Loader classes in the SDK and appcompat-v7 versions of the Activity classes.

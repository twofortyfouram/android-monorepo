...
if [[ $gcloud_exit_code != 0 && $gcloud_exit_code != 10 ]]
    then
    echo "gcloud failed with $gcloud_exit_code"
    exit $gcloud_exit_code
fi

# Due to discussion on the Firebase slack, try sleeping a few seconds before pulling the results.
# Google Cloud is a distributed filesystem, and it is possible for results not 
# to be visible yet after gcloud exists.
sleep 5
...
gsutil_exit_code=$?
echo "gsutil fetching test results finished $gsutil_exit_code"
...
gsutil_exit_code=$?
echo "gsutil fetching coverage finished $gsutil_exit_code"